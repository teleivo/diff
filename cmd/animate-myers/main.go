...
func run(args []string, w io.Writer, in io.Reader, wErr io.Writer) (int, error) {
	...
	flags.Usage = func() {
		_, _ = fmt.Fprintln(wErr, "animate-myers animates the quadratic space Myers diff algorithm on an edit graph.")
		_, _ = fmt.Fprintln(wErr, "")
		_, _ = fmt.Fprintln(wErr, "It shows how the algorithm explores the edit graph by incrementing the edit")
		_, _ = fmt.Fprintln(wErr, "distance d. For each d, the furthest reaching point on each diagonal is shown.")
		_, _ = fmt.Fprintln(wErr, "Not all paths the algorithm considers are shown, only the ones that advance the")
		_, _ = fmt.Fprintln(wErr, "furthest. The final frame shows the shortest edit script found by backtracking.")
		_, _ = fmt.Fprintln(wErr, "")
		_, _ = fmt.Fprintln(wErr, "Edges: → delete (dashed), ↓ insert (dotted), ↘ equal (solid diagonal)")
		_, _ = fmt.Fprintln(wErr, "")
